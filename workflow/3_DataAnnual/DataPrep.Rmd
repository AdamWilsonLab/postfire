...
## get the list of files and bands in correct temporal order
#nfiles=lapply(1:nlayers(undvi), function(l) undvi[[l]]@file@name)[order(getZ(undvi))]
#nbands=lapply(1:nlayers(undvi), function(l) undvi[[l]]@data@band)[order(getZ(undvi))]

ndvi=undvi[[order(getZ(undvi))]]
rm(l4,l5,l7,l8,undvi)

## create a new ndvi layer 
ndvifile="data/ndvi_landsat_30m.tif"
if(!file.exists(ndvifile)){

#ndvi=stack(lapply(1:nlayers(undvi),function(l) raster(nfiles[[l]],band=nbands[[l]])))
writeRaster(ndvi,filename=ndvifile)

}
```
  Note that this stack is not temporally sorted, so all LANDSAT 4 dates are before LANDSAT 5, etc.
