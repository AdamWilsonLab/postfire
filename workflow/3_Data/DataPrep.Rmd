...
Today, we'll simply take the mean of available data for each year.  
...
if(!file.exists(ndvifile)){
  ...
  ndvi=foreach(y=ndates,.combine=stack,.packages="raster") %dopar% {
    ...
    # this will average across sensors if there is duplicate data
...
write.csv(ndates,file="data/ndates.csv",row.names=F,col.names=F)

ndvi=stack(ndvifile)

ndvi=stack(l4,l5,l7,l8)
ndvi=setZ(ndvi,c(getZ(l4),getZ(l5),getZ(l7),getZ(l8)))
ndvi
```
